local ComplexProjectile_Hamlet = Class(function(self, inst)
	self.inst = inst
...
function ComplexProjectile_Hamlet:GetDebugString()
	return tostring(self.velocity)
end

function ComplexProjectile_Hamlet:SetHorizontalSpeed(speed)
    self.horizontalSpeed = speed
end

function ComplexProjectile_Hamlet:SetGravity(g)
    self.gravity = g
end

function ComplexProjectile_Hamlet:SetLaunchOffset(offset)
    self.launchoffset = offset -- x is facing, y is height, z is ignored
end

function ComplexProjectile_Hamlet:SetOnLaunch(fn)
	self.onlaunchfn = fn
end

function ComplexProjectile_Hamlet:SetOnHit(fn)
	self.onhitfn = fn
end

function ComplexProjectile_Hamlet:GetHorizontalSpeed(distance)
	return self.horizontalSpeed or Lerp(3, 12, distance/self.maxRange)
end

function ComplexProjectile_Hamlet:GetVerticalVelocity(distance)
	return ((self.gravity * distance)/2)/self:GetHorizontalSpeed(distance)
end

function ComplexProjectile_Hamlet:CalculateTrajectory(startPos, endPos, speed)
...
function ComplexProjectile_Hamlet:Launch(targetPos, attacker)
...
function ComplexProjectile_Hamlet:Hit(target)
...
function ComplexProjectile_Hamlet:OnUpdate(dt)
...
return ComplexProjectile_Hamlet